...
The compact JSON outputs must be byte-for-byte identical. The Node harness also checks source validation, raw-byte hashing, UTF-8 and NUL skips, Markdown/frontmatter/wikilinks, CRLF handling, heading chunking and overlap, technical identifiers, path-derived rename identity, string-encoded nanoseconds, portable API/status serialization, ordinary/explicit/identifier query plans with backend-neutral Boolean intent, fixed lowercase metadata probes, and inert SQL-looking input.
...
```json
{"status":"pass","cases":23,"native_bytes":12345,"wasm_bytes":12345,"byte_identical":true}
```
