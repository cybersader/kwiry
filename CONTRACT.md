...
## 10d. Lexical-v2 field-aware relevance (amendment, 2026-08-24)
...
2. **Shared text-evidence policy.** Shared Rust owns a fixed, versioned, integer scorecard over closed field classes and field-coherent proof classes. Strong source-identity evidence may outrank body-only evidence across legacy lexical-v1 tiers. The strongest coherent proof is used; duplicate filename, stem, alias, or title projections are not summed.
...
7. **Metadata boundary.** Recency, hierarchy, path, format, typed properties, and source-specific mappings remain unable to cross lexical-v2 text-evidence bands. Expanding those signals, changing the public field set, accepting numeric weighting, or changing the fixed scorecard requires a separately reviewed amendment and judged evidence.
8. **Compatibility and privacy.** The amendment changes query-plan and execution-plan identities, not source preparation, chunk IDs, index/cache schema, tokenizer identity, or stored rows. It therefore requires no content reindex by itself. Safe diagnostics and disclosures may contain only closed enums, versions, counts, bounds, and ordinals—never query text, paths, source names, excerpts, SQL, raw plans, or engine exceptions.
