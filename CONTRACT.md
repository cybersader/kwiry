...
6. **Imported retrieval algorithms; bounded project-owned relevance policy.** Lexical scoring, ANN, tokenization, and embedding remain imported. Kwiry may apply deterministic, bounded, source-neutral policies after imported lexical retrieval. The compatibility profile `lexical-v1` may reorder candidates only within the same legacy lexical-evidence tier. The versioned `lexical-v2` profile may instead compare shared-Rust, field-coherent text-evidence proofs across those legacy tiers, including allowing strong filename/title identity evidence to outrank body-only phrase evidence. Recency, hierarchy, path, format, typed properties, source-specific mappings, and other metadata may still refine only equally strong `lexical-v2` text-evidence bands; they may not cross those bands without another owner-approved amendment. No relevance policy may widen authorization, replace imported candidate matching or BM25, or compare raw Tantivy and FTS5 scores across engines. Shared Rust owns the model, public field-control grammar, validation, limits, scorecard, and ordering; adapters only map authorized source data and execute the declared bounded plan. The in-plugin host may bind an allowlisted query plan to fixed parameterized FTS5 SQL and fixed weight vectors, but TypeScript does not implement tokenization or a bespoke lexical ranker. RRF fusion of ranked lists remains permitted as a bounded formula.
...
## 10d. Lexical-v2 field-aware relevance (amended 2026-09-05)

1. **Named compatibility boundary.** `lexical-v2` becomes the ordinary lexical profile for desktop, in-plugin, and OpenClast lexical requests. `lexical-v1` retains its tier-monotonic meaning as an explicit compatibility identity; it is not silently redefined.
2. **Shared text-evidence policy.** Shared Rust owns a fixed, versioned, integer scorecard over closed field classes and field-coherent proof classes. Standard proofs and candidates always precede partial-coverage proofs and candidates. Within each class, the existing integer scorecard and fixed field emphasis determine order. A candidate carrying both classes selects its strongest standard proof; otherwise it selects its strongest proof within its available class. Strong source-identity evidence may outrank body-only evidence across legacy lexical-v1 tiers. Duplicate filename, stem, alias, or title projections are not summed.
3. **Imported engines remain authoritative.** Tantivy and FTS5 continue to own tokenization, matching, and BM25. Engine-local scores may order candidates only inside comparable proof lanes. Returned hit scores remain imported engine scores, and raw scores are never compared across engines.
4. **Bounded candidate work.** Hosts execute only Rust-declared allowlisted proof lanes under fixed per-lane, lane-count, and unique-candidate ceilings. Authorization constrains physical retrieval and scoring statistics before any lane executes. Deduplication, final ordering, and truncation occur in shared Rust.
5. **Public controls.** Leading `in:<field>` selects one strict lexical field scope and leading `><field>` selects one fixed bounded emphasis step. The closed public fields are `name`, `filename`, `title`, `alias`, `heading`, `tag`, and `body`; `name` groups filename, stem, title, and aliases. Controls are parsed and validated in shared Rust, cannot widen authorization, and never expose native engine schema names. Arbitrary numeric boosts, sliders, path scope, recursive-property scope, and source-authored ranking instructions are not admitted.
6. **Mode behavior.** Lexical mode accepts scope and emphasis. Hybrid mode accepts emphasis only for its lexical leg and embeds the stripped query text; strict scope is rejected because it cannot constrain the semantic leg truthfully. Semantic-only mode rejects all lexical controls. Invalid, unsupported, or incompatible controls fail explicitly without silent stripping, profile fallback, mode fallback, or backend switching.
7. **Metadata boundary.** Recency, hierarchy, path, format, typed properties, and source-specific mappings remain unable to cross lexical-v2 text-evidence bands. The standard-before-partial class boundary is part of the fixed scorecard. Expanding those signals, changing the public field set, accepting numeric weighting, or changing either that class boundary or the within-class scorecard requires a separately reviewed amendment and judged evidence.
8. **Compatibility and privacy.** The 2026-08-24 amendment changes query-plan and execution-plan identities, not source preparation, chunk IDs, index/cache schema, tokenizer identity, or stored rows. The 2026-09-05 amendment changes only the internal lexical-v2 rank schema and its exact host/adapter mirrors from version 1 to version 2; query-plan version 12, FTS5 execution-plan version 11, source preparation, chunk IDs, persisted index/cache state, tokenizer identity, stored rows, and the release version remain unchanged. Neither amendment requires a content reindex by itself. Safe diagnostics and disclosures may contain only closed enums, versions, counts, bounds, and ordinals—never query text, paths, source names, excerpts, SQL, raw plans, or engine exceptions.
