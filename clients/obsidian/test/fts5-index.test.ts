...
vi.mock("../src/worker/rust-adapter", () => ({
  ...
  finalizeLexicalV2RankWithRust: (input: {
    ...
    candidates: Array<{
      proofs: Array<{ engine_score: number; kind: string }>;
    }>;
  }) => {
    if (input.schema_version !== 2) {
      throw new Error("unexpected lexical-v2 rank schema");
    }
    return {
      ...
      selected_proof_kinds: input.candidates.map((candidate) =>
        candidate.proofs[0]?.kind ?? "all_terms"),
    };
  },
}));
...
describe("Fts5GenerationIndex", () => {
  ...
  it("executes every fallback-only partial lane after standard lanes return zero", () => {
    index.replaceSource(sourceAt(
      "body-partial",
      "body-partial.md",
      "chunk-body-partial",
    ...
    expect(result.lexical_match_quality).toBe("partial_only");
    expect(summary).toMatchObject({
      planned_lane_count: 3,
      executed_lane_count: 3,
      unique_candidate_count: 2,
    });
    expect(summary.lane_kinds).toContainEqual(expect.objectContaining({
      key: "lexical_partial_coverage_v3",
      planned_lane_count: 2,
      executed_lane_count: 2,
    }));
  });

  it("keeps a title prefix hit ahead of body-text prefix hits", () => {
    index.replaceSource(sourceAt(
      "titled-zorb",
      "pinnacle-target.md",
      "chunk-titled-zorb",
      "Synthetic selection target",
      "Quarry Zorbification",
    ));
    for (let value = 0; value < 24; value += 1) {
      const suffix = String(value).padStart(2, "0");
      index.replaceSource(sourceAt(
        `bodied-zorb-${suffix}`,
        `pinnacle-decoy-${suffix}.md`,
        `chunk-bodied-zorb-${suffix}`,
        "Quarry zorbification body evidence",
        "Quarry",
      ));
    }
    ...
    // Twenty-four body matches cannot crowd out the one titled match, because
  ...
  it("observes support only in canonical fields and bounds sorted prefix terms", () => {
    ...
    const observed = index.observeQuery([{
      ...
      prefix_fields: ["filename", "stem", "aliases", "title", "heading", "tags", "content"],
      prefix_stem: "quasar",
      max_prefix_expansions: 16,
      max_prefix_expansion_scan: 256,
      max_prefix_term_bytes: 96,
