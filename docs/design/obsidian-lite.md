...
The no-daemon host remains contractual but not delivered. Gates 1–4 are owner-accepted GO. The Gate 5 candidate on PR #2 implements bounded active-vault indexing/reconciliation and its automated/package evidence, but remains unmerged under the owner's field-first sequence. Generated performance meets the build, warm-search, update-visibility, and event-loop targets while missing the provisional 300 MiB added-memory target; installed restart/update/rollback and declared-reference-hardware acceptance remain open.
...
### Gate 5 — active-vault lifecycle and acceptance — Candidate implemented, field acceptance pending

PR #2 implements atomic initial build plus create/modify/delete/rename reconciliation, bounded stable reads and event accumulation, exact-Worker/package evidence, and deterministic functional/performance corpora. The installed Obsidian restart/update/rollback sequence, private aggregate-only evidence, declared-reference-hardware measurements, and explicit owner acceptance remain before a delivered claim.

### Gate 6 — durable cache and differential warm start — Approved, feasibility pending
...
Before changing the Worker protocol, the exact generated 10,000-note/50-MiB harness must prove official SQLite export/deserialize integrity, restore speed, event-loop behavior, and bounded peak RSS. A NO-GO stops integration rather than adding OPFS, another Worker, another WASM payload, or a custom SQLite build.
...
The Gate 5 candidate is desktop-only and Markdown-only:

1. register vault events before taking the initial snapshot;
2. enumerate Markdown files and read stable source bytes through Obsidian APIs;
