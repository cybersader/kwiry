...
- when the fixed 100-section source-discovery response window itself comes back full, disclose (query-free) that additional sources may be unobserved beyond that window — a third, closed fact kept independent of both source-row truncation (the exact, locally-known count omitted by the source-row limit) and candidate-window completeness (the backend's own scan-completeness signal);
...
- **Lexical-v2 text evidence:** shared Rust owns a fixed, versioned scorecard over field-coherent proofs and bounded cross-lane candidate collection. The 2026-09-05 amendment advances the internal rank schema to version 2: standard proofs and candidates always precede partial-coverage proofs and candidates, while the existing integer evidence points and fixed field emphasis order candidates within each class. A candidate carrying both classes selects its strongest standard proof. Strong filename/title identity may outrank body-only phrase evidence within the standard class. `lexical-v1` remains a compatibility identity, imported engines retain tokenization/matching/BM25, and cross-engine acceptance uses judged partial orders rather than raw-score equality.
...
### Bounded outcome-gated best attempt

For an ordinary assistance-eligible query with at least two useful optional-context terms, shared Rust may declare one bounded exploratory `partial_coverage` stage. One useful optional term does not get a redundant fallback. Execution is outcome-gated: the stage runs only when authorized standard lanes reach fewer than 20 distinct qualified sources, the decision counts sources rather than chunks, it is latched before the first conditional partial lane, and exactly 20 sources suppress it. Every required identifier anchor remains mandatory. Each included optional term contributes one alternative: when the canonical standard prefix stages carry a bounded expansion set for that term, the partial stage reuses that same group; otherwise it uses the analyzed exact form. At least one optional alternative must match (`minimum_optional_matches: 1`), with one flat `OR` rather than enumerated combinations or sampled subsets. Native Tantivy compiles anchors as `MUST` and optional clauses as a minimum-required-1 `SHOULD` group. FTS5 MATCH compiles analyzed anchors conjunctively with one field-scoped `OR` over exact alternatives and bounded expansion groups, while exact-identifier anchors remain a separate mandatory intersection. OpenClast consumes the same declared condition, threshold, and term partition. Retrieval stops before later lanes once the fixed 512-unique-candidate window is full, while preserving the 256-raw-row limit for the lane that fills it so cross-lane duplicates do not reduce unique recall.

Interactive search uses a 100 ms trailing-edge scheduler before the backend boundary. It permits at most one active backend search and one replaceable latest pending search; superseded pending work resolves stale, and a superseded active request may finish only before the latest due query dispatches. This bounds synchronous Worker queue amplification without claiming that an active SQLite search can be cancelled.

Exact metadata, exact phrase, prefix, and complete all-term lanes remain primary. The exploratory decision is made once before the conditional partial stage: fewer than 20 distinct authorized standard sources activates all of its bounded field lanes, while 20 or more suppresses them. Existing partial coverage caused by unsupported optional context remains an always-executed supplemental stage. Explicit syntax, authored phrase/exclusion bypasses, field controls, required identifier anchors, authorization-before-retrieval, per-stage candidate ceilings, and the total retained-candidate ceiling are unchanged.
