...
For a fully supported ordinary query with at least two useful optional-context terms, shared Rust may declare one bounded exploratory partial-coverage pass; one optional term remains unchanged because the fallback would duplicate the standard requirement. Every identifier anchor stays mandatory. Each useful optional term contributes either the same canonical bounded prefix-expansion group used by standard lanes or, when no expansion exists, its analyzed exact form, and at least one optional alternative must match. The pass activates only when prior authorized standard lanes reach fewer than 20 distinct qualified sources and is suppressed at exactly 20. Native, in-plugin, and OpenClast execution consume the same condition, fixed threshold, and exact/prefix term partition. Later retrieval lanes stop once the fixed 512-unique-candidate window fills, without reducing the filling lane's fixed 256-row raw bound. Interactive typeahead uses a 100 ms trailing-edge scheduler with at most one active request and one replaceable latest pending request. No combinatorial subset search, explicit-syntax relaxation, authorization widening, silent mode fallback, or claimed cancellation of active synchronous Worker execution is permitted.

Query-assistance status is split: zero-result copy, beta.27 field controls, and mandatory best-attempt/mixed lexical result disclosure are accepted; typo assistance remains prototype-only pending performance and visible-limitation evidence. Search status also discloses, query-free, when the fixed 100-section source-discovery response window itself comes back full — a closed fact kept independent of both the source-row limit's exact omission count and candidate-window completeness.
