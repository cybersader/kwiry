...
Shared Rust owns the bounded evidence plan and final scorecard across both engines. A fully supported ordinary query with at least two useful optional terms may declare one bounded exploratory partial-coverage pass; one optional term remains unchanged because an any-term fallback would be redundant. The pass executes only when stronger authorized lanes reach fewer than 20 distinct qualified sources. Required identifier anchors stay mandatory, and each useful optional term contributes one alternative: its canonical bounded prefix-expansion group when available, otherwise its analyzed exact form. At least one alternative must match, using one flat disjunction rather than enumerated combinations or sampled subsets. Results selected from that pass are visibly labeled as best-attempt partial matches; mixed standard/partial result windows are labeled separately. Retrieval stops before later lanes once 512 unique candidates are retained, and interactive typeahead permits at most one active backend request plus one replaceable latest pending request after a 100 ms trailing edge. Explicit syntax, required identifier anchors, authorization, mode availability, and existing candidate ceilings are not relaxed.
